  ...
  const bindEvents = (cesiumObject, cesiumEvents, register = true) => {
    ...
    ev && ev.forEach((eventName) => {
      if (cesiumObject[eventName]) {
        const listener = vm.getInstanceListener(vcInstance, eventName);
  ...
  const registerEvents = (register) => {
    ...
    if (!viewer._vcPickScreenSpaceEventHandler || !viewer._vcViewerScreenSpaceEventHandler) {
      ...
      viewerScreenSpaceEvents.forEach((type) => {
        const listener = vm.getInstanceListener(vcInstance, type);
    ...
    (_a = vcInstance.cesiumMembersEvents) == null ? void 0 : _a.forEach((eventName) => {
      const cesiumIntanceMember = shared.isArray(eventName.name) && eventName.name.length > 0 && cesiumObject[eventName.name[0]] ? cesiumObject[eventName.name[0]][eventName.name[1]] : cesiumObject[eventName.name];
  ...
  function pickedAction(movement) {
    ...
    pickedFeatureAndCallbackNames.forEach((item) => {
      ...
      if (pickedFeature2.id) {
        if (shared.isArray(pickedFeature2.id) && pickedFeature2.id[0] instanceof Cesium.Entity) {
    ...
    eventSourceList.forEach((event) => {
      if (event.callbackName) {
        const fn = event.cesiumObject[event.callbackName] || event.cesiumObject[`on${shared.capitalize(event.callbackName)}`] || event.cesiumObject[util.kebabCase(`on${shared.capitalize(event.callbackName)}`)];
        fn && fn({
...
exports.pickEvents = pickEvents;
exports.viewerScreenSpaceEvents = viewerScreenSpaceEvents;
