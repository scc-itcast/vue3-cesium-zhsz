...
var index$1 = require('../use-events/index.js');
...
function useCommon(props, { emit }, vcInstance) {
  ...
  const $services = vue.inject(config.vcKey);
  const { t } = index.useLocale();
  if ($services === void 0) {
    console.error(`${vcInstance.cesiumClass} ${t("vc.loadError")}`);
    return;
  }
  const parentVcInstance = vm.getVcParentInstance(vcInstance);
  ...
  const createCesiumObject = async () => {
    ...
    if (shared.isFunction(vcInstance.createCesiumObject)) {
  ...
  const deepWatchHandler = (vueProp, watcherOptions) => {
    ...
    if (vueProp === "position") {
      ...
      deep = !(value instanceof Appearance || value instanceof DebugAppearance || value instanceof MaterialAppearance || value instanceof PolylineColorAppearance || value instanceof EllipsoidSurfaceAppearance || value instanceof PerInstanceColorAppearance || value instanceof PolylineMaterialAppearance || util.getObjClassName(value).indexOf("Appearance") !== -1);
    }
    return deep;
  };
  ...
  const transformProps = (props2, childProps) => {
    ...
    props2 && Object.keys(props2).forEach((vueProp) => {
      ...
      const className = util.getObjClassName(props2[vueProp]);
  ...
  const transformProp = (prop, value, childProps) => {
    var _a, _b;
    const className = util.getObjClassName(value);
    if (className && className.indexOf("Graphics") === -1 && entityGraphics.indexOf(prop) !== -1 && (vcInstance.cesiumClass === "Entity" || vcInstance.cesiumClass.indexOf("DataSource") > 0 || vcInstance.cesiumClass === "VcOverlayDynamic")) {
      ...
      return (propOption == null ? void 0 : propOption.watcherOptions) && !util.isEmptyObj(value) ? propOption.watcherOptions.cesiumObjectBuilder.call(vcInstance, value, vcInstance.viewer.scene.globe.ellipsoid) : shared.isFunction(value) && cmpName && (cmpName.indexOf("Graphics") !== -1 || cmpName === "VcEntity" || cmpName.indexOf("Datasource") !== -1) ? new Cesium.CallbackProperty(value, false) : value;
    }
  };
  const getServices = () => {
    return mergeDescriptors.mergeDescriptors({}, $services || {});
  };
