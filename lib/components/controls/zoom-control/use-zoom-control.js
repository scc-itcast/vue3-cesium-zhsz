...
function useZoomControl(props, { emit }, vcInstance, $services) {
    ...
    if (defined(viewer)) {
      ...
      switch (scene.mode) {
        ...
        default: {
          ...
          const level = cesiumHelpers.heightToLevel(camera.positionCartographic.height).toFixed(0);
  ...
  const zoomReset = (e) => {
    var _a;
    (_a = vm.$(resetTooltipRef)) == null ? void 0 : _a.hide();
    const { viewer } = $services;
    const scene = viewer.scene;
    const sscc = scene.screenSpaceCameraController;
    ...
    if (Cesium.defined(viewer.trackedEntity)) {
      ...
      const listener = vm.getInstanceListener(vcInstance, "zoomEvt");
      const target = e.currentTarget;
      const level = cesiumHelpers.heightToLevel(viewer.camera.positionCartographic.height).toFixed(0);
      listener && emit("zoomEvt", {
        type: "zoomReset",
        camera: viewer.camera,
        status: "start",
      ...
      cesiumHelpers.flyToCamera(viewer, resetView, options);
    }
  };
