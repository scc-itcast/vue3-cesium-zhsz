...
function regularGrid(\u03BBaxis, \u03C6axis) {
  ...
  const np = nx * ny;
  const \u0394\u03BB = math.decimalize(\u03BBaxis.delta);
  const \u0394\u03C6 = math.decimalize(\u03C6axis.delta);
  const \u03BB0 = math.decimalize(\u03BBaxis.start);
  const \u03C60 = math.decimalize(\u03C6axis.start);
  const \u03BB1 = \u03BB0 + \u0394\u03BB * (nx - 1);
  const \u03C61 = \u03C60 + \u0394\u03C6 * (ny - 1);
  const \u03BBlow = (\u03BB0 - \u0394\u03BB / 2) * consts.RAD;
  const \u03BBhigh = (\u03BB1 + \u0394\u03BB / 2) * consts.RAD;
  const \u03BBsize = \u03BBhigh - \u03BBlow;
  const \u03C6low = (\u03C60 - \u0394\u03C6 / 2) * consts.RAD;
  const \u03C6high = (\u03C61 + \u0394\u03C6 / 2) * consts.RAD;
  ...
  function closest(\u03BB, \u03C6) {
    if (\u03BB === \u03BB && \u03C6 === \u03C6) {
      const x = math.floorMod(\u03BB - \u03BB0, 360) / \u0394\u03BB;
      const y = (\u03C6 - \u03C60) / \u0394\u03C6;
...
exports.regularGrid = regularGrid;
